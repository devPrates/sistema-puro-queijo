...
import { ProdutoUpdateFormValues, ProdutoUpdateSchema } from "../actions/produto-schema"
import { updateProduto } from "../actions/produto-actions"

interface EditProdutoDialogProps {
  produto: ProdutoWithRelations
}

export function EditProdutoDialog({ produto }: EditProdutoDialogProps) {
  const [open, setOpen] = useState(false)
  const { usuarios, isLoading: isLoadingUsuarios } = useUsuarios()
  const { categorias, isLoading: isLoadingCategorias } = useCategorias()

  const form = useForm<ProdutoUpdateFormValues>({
    ...
    defaultValues: {
      ...
      tamanhoEmbalagem: produto.tamanhoEmbalagem,
      categoriaId: produto.categoriaId.toString(),
      usuarioId: produto.usuarioId.toString(),
    },
  })
  ...
  async function onSubmit(data: ProdutoUpdateFormValues) {
    try {
      await updateProduto({
        ...data,
